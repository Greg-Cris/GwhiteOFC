...
export default function HomePage() {
  ...
  const ServerCard = ({ server }) => {
    const symbols = ['🎵', '🎶', '♪', '♫', '🎧', '🎤'];
    const symbolElements = [...Array(8)].map((_, i) => {
      ...
      return { leftPos, topPos, duration, delay, symbol };
    });
          ...
          {symbolElements.map((el, i) => (
            <div
              ...
              style={{
                left: `${el.leftPos}%`,
                top: `${el.topPos}%`,
                animation: `float-symbol ${el.duration}s infinite ease-in-out`,
      ...
      <main className="flex flex-col items-center justify-center px-6 py-20 pt-32 text-center min-h-screen relative z-10">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <button className="bg-gradient-to-br from-amber-600 to-amber-800 hover:from-amber-700 hover:to-amber-900 text-white px-8 py-4 text-lg rounded-lg font-bold shadow-xl transform hover:scale-105 transition-all duration-200">
              Ir para a Dashboard →
            </button>
            <Link href="/planos">
